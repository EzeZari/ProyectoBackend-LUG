...
const productController = {
    ...
    get: async (req: Request, res: Response) => {
        try{
            const findProducts = await productModel.find() //La var es = al metodo de busqueda find
    ...
    getunique: async (req: Request, res: Response) => {
        try{
            const findProductsUnique = await productModel.findOne({... req.params}) //Si la const "buscar producto unico es = al metodo find"
            if(findProductsUnique?.Name_Product != undefined){                     //"buscar prod unico" accede al nombre del producto y este tiene que ser distinto de undefined
    ...
    delete: async (req: Request, res: Response) => {
        try{
            ...
            if(findProducts?.Name_Product != undefined || findProducts?.Name_Product != null){   //    ||=or
